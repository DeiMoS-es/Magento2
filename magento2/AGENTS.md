...
## 🗂 Plantilla mínima de módulo (ejemplo: `Prueba_HelloWorld`)

Cuando se solicite "crear un módulo", el agente usará por defecto la estructura mínima que se encuentra en `app/code/Prueba/HelloWorld/` (ejemplo de referencia). Esta sección documenta la estructura y los pasos operativos para crear y activar un módulo mínimo.

### Estructura mínima que se genera por defecto

- `app/code/<Vendor>/<Module>/registration.php` — registra el módulo con `ComponentRegistrar`.
- `app/code/<Vendor>/<Module>/etc/module.xml` — declaración del módulo y dependencias (opcional).
- `app/code/<Vendor>/<Module>/composer.json` — (opcional) metadata y autoload PSR-4.
- `app/code/<Vendor>/<Module>/Controller/Index/Index.php` — (opcional) controlador de ejemplo para probar ruta frontend.
- `app/code/<Vendor>/<Module>/README.md` — instrucciones de activación y prueba.

### Comandos habituales tras creación (ejecutar en el entorno que tiene acceso a la DB y Redis; usar Warden/container PHP)
...
- Usar `declare(strict_types=1);` y tipado en firmas cuando sea posible.
- Inyectar dependencias vía constructor (`__construct`) y evitar `ObjectManager::getInstance()`.
- Seguir PSR-12 y PSR-4 en nombres y estructura.
- Para lógica persistente usar el patrón ResourceModel/Model/Api/Data/Repository.
- Añadir un `README.md` en el módulo que explique los pasos para activar y probar.

Si el usuario quiere una plantilla diferente, preguntar antes de crear el módulo y adaptarla.
# 🎯 OBJETIVO DEL AGENTE: Constructor de Módulos de Magento 2 (Deimos Vendor)
